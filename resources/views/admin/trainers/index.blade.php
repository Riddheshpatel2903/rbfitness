...
<a href="{{ route('admin.trainers.create') }}" class="btn btn-primary"><i class="fas fa-plus"></i> Add Trainer</a>
@endsection
...
<div class="card">
    <div class="filter-container" style="margin-bottom: 2rem;">
        ...
        <span id="total-count" style="opacity:0.45;font-size:0.82rem;white-space:nowrap;">{{ $trainers->count() }} total trainers</span>
    </div>
    ...
    <div class="table-responsive">
        ...
        <tbody id="table-body">
            @include('admin.trainers._table', ['trainers' => $trainers])
        </tbody>
    </table>
</div>
@endsection
...
(function () {
    ...
    const BASE_URL = '{{ route("admin.trainers.index") }}';
    const DELETE_BASE = '{{ url("rbadmin/trainers") }}';
    ...
    function fetchTrainers(search = '') {
        ...
        .then(data => {
            ...
            totalCount.textContent = `${data.total} total trainers`;
    ...
    searchInput.addEventListener('input', () => {
        ...
        searchTimer = setTimeout(() => {
            fetchTrainers(searchInput.value.trim());
        }, 300);
    });
            ...
            btn.addEventListener('click', function() {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        showToast(data.message || 'Error removing trainer.', 'error');
                    }
                })
                .catch(err => {
                    row.style.opacity = '1';
